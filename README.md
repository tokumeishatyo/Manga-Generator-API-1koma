# AI創作工房 (Manga-Generator-API-1koma)

Google Gemini API (`gemini-3-pro-image-preview`) を利用した、高品質イラスト生成デスクトップアプリケーションです。
キャラクターデザイン、ポーズ生成、背景作成、シーン合成まで幅広い創作をサポートします。

詳しい使い方や開発背景については、[Qiita記事](https://qiita.com/KLaboratory/items/77b81226d1db8089c7fc)をご覧ください。

## 特徴

- **最新AIモデル**: `gemini-3-pro-image-preview` (Gemini 3 Pro) を使用
- **3つの出力モード**:
  - **YAML出力**: ブラウザ版Geminiにコピペして使用
  - **API画像出力**: アプリ内で直接画像生成（清書モード対応）
  - **シンプルモード**: 設定不要、テキストプロンプトのみで画像生成
- **清書モード**: ブラウザで作成した画像+YAMLをAPIで高品質再描画
- **画像加工機能**: 生成した画像を全体改善または部分修正で再加工
- **API使用量トラッキング**: 今日/今月の使用回数をリアルタイム表示
- **キャラクター参照画像**: 画像を添付して顔や外見の一貫性を保持
- **服装ビルダー**: ドロップダウンで服装を選ぶだけで英語プロンプトを自動生成
- **多彩な出力モード**:
  - キャラデザイン（全身三面図/顔三面図）
  - ポーズ付きキャラ
  - エフェクト追加
  - 背景生成
  - 装飾テキスト（技名/決め台詞/キャラ名プレート/メッセージウィンドウ）
- **シーンビルダー**: バトル/ストーリー（最大3人）/ボスレイドのシーン合成
- **背景プロンプト生成**: ファイル指定または情景説明で背景を生成
- **キャラクター特徴**: 髪型・身長などでAIにキャラを識別させる
- **装飾テキスト配置**: 位置・サイズ・レイヤーを自由に設定可能
- **4コマ漫画**: 起承転結の4コマ漫画を一括生成
- **漫画ページコンポーザー**: 生成した画像を組み合わせて漫画ページを作成
- **カラーモード**: フルカラー / モノクロ / セピア / 二色刷り
- **出力スタイル**: 漫画風、アニメ風、劇画風、水彩風、アメコミ風など多数
- **YAML入出力**: プロンプトの保存・読み込み・再編集が可能
- **タイトル機能**: ファイル名自動生成、画像へのタイトル合成（オプション）
- **進捗表示**: API生成中の経過時間表示

## 生成ワークフロー

```
1. キャラデザイン（三面図）
   ↓ 参照画像として使用
2. ポーズ付きキャラ
   ↓ ポーズ付き画像として使用
3. エフェクト追加（オプション）
   ↓ エフェクト付き画像として使用
4. シーンビルダーで合成
   - バトルシーン（2キャラ対戦）
   - ストーリーシーン（会話/日常、最大3人）
   - ボスレイド（巨大ボス vs パーティ）
   + 背景: ファイル指定 or プロンプト生成
   + キャラ特徴: 髪型・身長で識別
   + 装飾テキスト配置（位置・サイズ・レイヤー）

## 清書モード（API高品質再描画）

ブラウザ版Geminiで試行錯誤して作成した画像を、APIでより高品質に再描画する専用モードです。

```
1. ブラウザ版Geminiで画像を生成（試行錯誤）
2. 成功したYAMLをファイルに保存
3. アプリでAPI出力モード →「参考画像清書」を選択
4.「読込」ボタンでYAMLを読み込む
5. 参考画像（ブラウザで生成した画像）を選択
6. タイトル入力（ファイル名用）
7. 解像度選択（1K/2K/4K）
8. 生成ボタン → 高品質再描画実行
```

**特徴:**
- **YAML + 参照画像**の両方を使用
- YAMLの指示に従いつつ、参照画像の構図を再現
- より詳細で高品質な描き込みで再描画
- 保存ファイル名に「_API」サフィックス自動付与

## 画面レイアウト

```
┌───────────────────────────────────────────────────────────────────────────────┐
│  左列（設定）          │  中列（API）           │  右列（プレビュー）        │
├────────────────────────┼────────────────────────┼────────────────────────────┤
│ ・出力タイプ選択       │ ・API設定              │  ・YAMLプレビュー          │
│   - キャラデザイン     │   - 出力モード         │    [コピー][保存][読込]    │
│   - ポーズ付きキャラ   │     (YAML/API)         │                            │
│   - エフェクト追加     │   - API Key [×]       ├────────────────────────────┤
│   - 背景のみ生成       │   - APIモード          │  ・画像プレビュー          │
│   - 装飾テキスト       │     (通常/清書)        │    (生成中: 経過時間表示)  │
│   - 4コマ漫画         │   - 参考画像           │    [画像保存]              │
│ ・詳細設定ボタン       │   - 解像度(1K/2K/4K)   │                            │
│                        │                        │                            │
│ ・スタイル設定         │                        │                            │
│   - カラーモード       │                        │                            │
│   - 出力スタイル       │                        │                            │
│   - アスペクト比       │                        │                            │
│                        │                        │                            │
│ ・基本情報             │                        │                            │
│   - タイトル（必須）   │                        │                            │
│   - □タイトルを入れる │                        │                            │
│   - 作者名             │                        │                            │
│                        │                        │                            │
│ [YAML生成] [リセット]  │                        │                            │
│ [シーンビルダー]       │                        │                            │
│ [漫画ページコンポーザー]│                        │                            │
└────────────────────────┴────────────────────────┴────────────────────────────┘
```

## 出力タイプ詳細

### キャラデザイン（三面図）
- **全身三面図**: 正面・側面・背面の設定画
- **顔三面図**: 正面・斜め・横顔の設定画
- キャラクタースタイル選択（標準アニメ/ちびキャラ/リアル調など）
- 服装ビルダーで詳細な服装指定

### ポーズ付きキャラ
- カメラアングル: 正面/斜め/俯瞰/煽り
- ポーズ: 立ち/座り/ジャンプ/攻撃など
- 表情: 笑顔/怒り/悲しみ/驚きなど
- 背景透過対応

**ズーム設定のTips**:
ズーム設定の効果はポーズとの組み合わせで変わります：

| ポーズタイプ | おすすめズーム設定 | 備考 |
|-------------|-------------------|------|
| 攻撃系（波動拳など） | 顔アップ | 腕の動きを含めるため実質バストアップになる |
| 静止系（瞑想など） | みぞおちから上、胸から上 | 座りポーズは体がコンパクトなのでズームが効きにくい |
| 全身動作（キックなど） | 全身、上半身 | 動きを見せるため広めの画角を推奨 |

※「バストアップ」指定はAIのセーフティフィルタで安定しない場合があります。「顔アップ」や「胸から上」を代替としてお試しください。

### エフェクト追加
- エフェクトタイプ: オーラ/炎/電撃/氷/光/闘気
- 強度: 弱/中/強
- 色設定

### 背景のみ生成
- プリセット: 学校/公園/街中/海辺/ファンタジーなど
- カスタム説明対応

### 装飾テキスト
4タイプから選択:
| タイプ | 説明 | 設定項目 |
|--------|------|----------|
| 技名テロップ | 必殺技名表示 | フォント/サイズ/グラデ色/縁取り/発光 |
| 決め台詞 | キャラの派手なセリフ | 表現タイプ/配色/回転/変形 |
| キャラ名プレート | 名前表示 | デザイン/回転 |
| メッセージウィンドウ | ゲーム風セリフ枠 | モード（フル/顔のみ/セリフのみ） |

### シーンビルダー
3つの合成タイプ:
| タイプ | 用途 | 主な設定 |
|--------|------|----------|
| バトルシーン | 2キャラ対戦 | 衝突タイプ/優勢側/境界エフェクト/画面揺れ |
| ストーリーシーン | 会話/日常（最大3人） | 配置パターン/距離感/ダイアログ |
| ボスレイド | 巨大ボス戦 | ボススケール/パーティ配置/集中砲火 |

**背景設定** (2モード):
- ファイル指定: 既存の背景画像を使用
- 情景説明で生成: プロンプトで背景を生成（例: 「ゴージャスなパーティ会場」）

**キャラクター特徴** (`physical_traits`):
- 各キャラに髪型・身長などの特徴を指定可能
- 素体や同じ制服でもAIがキャラを正確に識別
- 例: 「長い黒髪、身長が一番高い」

**装飾テキスト配置** (最大10箇所):
- 位置: 自由入力（例: Top Center, Bottom Right）
- サイズ: 自由入力（例: 100%, 150%, 50%）
- レイヤー: 最前面/キャラの後ろ/背景の前

### 4コマ漫画
- 起承転結の4コマを一括生成
- キャラクター参照画像対応
- セリフ・ナレーション設定
- **画像生成指示文を自動追加**: ブラウザ版Geminiにコピペするだけで生成可能

### 漫画ページコンポーザー

生成した画像を組み合わせて漫画ページを作成するツールです。

| 機能 | 説明 |
|------|------|
| テンプレート | 4コマ（16:9×4縦並び）、8コマ（4:3×4の2列） |
| 画像配置 | 各コマに生成済み画像を選択 |
| 自動調整 | アスペクト比維持で中央クロップ＆リサイズ |
| 吹き出し追加 | 丸、角丸、ギザギザ（叫び）、雲（思考）の4スタイル |
| 出力サイズ | 720px / 1080px / 1440px（Web向け縦長） |

**使い方:**
1. メイン画面の「漫画ページコンポーザー」ボタンをクリック
2. テンプレートを選択（4コマ or 8コマ）
3. 各コマに画像を選択
4. 必要に応じて吹き出しを追加
5. 「プレビュー更新」で確認
6. 「画像を出力」で保存

## 必要要件

- **OS**: macOS / Windows / Linux
- **Python**: バージョン 3.10 以上
- **Google AI API Key**: [Google AI Studio](https://aistudio.google.com/apikey) で取得

## インストールと起動

### macOS / Linux

```bash
./run_app.command
```

### Windows

`run_app_windows.bat` をダブルクリック

初回起動時は自動的にセットアップ（仮想環境作成・依存ライブラリインストール）が行われます。

### macOS アプリ化（Automator）

Dockに置けるアプリとして起動したい場合、Automatorで.appを作成できます。

#### 作成手順

1. **Automator.app を開く**（Launchpadまたは `/Applications/Automator.app`）

2. **「新規書類」→「アプリケーション」を選択**

3. **左側のライブラリから「シェルスクリプトを実行」をダブルクリック**

4. **以下のスクリプトを入力:**
   ```bash
   cd "/path/to/Manga-Generator-API-1koma"  # ← 実際のパスに書き換え

   # venvがなければ作成
   if [ ! -d ".venv" ]; then
       python3 -m venv .venv
       source .venv/bin/activate
       pip install -r app/requirements.txt --quiet
   else
       source .venv/bin/activate
   fi

   python3 app/main.py
   ```
   ※ `/path/to/` の部分を実際のフォルダパスに変更してください

5. **Command + S で保存**
   - 名前: `MangaGenerator`（任意）
   - 場所: アプリケーション（または任意の場所）
   - フォーマット: アプリケーション

6. **作成した.appをDockにドラッグ**で登録完了

#### メリット

- ターミナルが開かない
- Dockからワンクリック起動
- Pythonコードを更新しても再作成不要（即反映）

#### アイコンの変更（任意）

1. 好きな画像を用意（.png, .jpg など）
2. プレビュー.appで開き、Command + C でコピー
3. 作成した.appを右クリック →「情報を見る」
4. 左上のアイコンをクリックして選択状態に
5. Command + V でペースト

### macOS スタンドアロンアプリ（PyInstaller）

Python環境に依存しない完全なスタンドアロンアプリを作成できます。

```bash
./build_app.sh
```

ビルド完了後、`dist/AI創作工房.app` が生成されます。

**注意**: ビルドはmacOS環境で行う必要があります（Docker/Linux環境ではmacOS用アプリは生成できません）。

## ディレクトリ構成

```
.
├── app/
│   ├── main.py                          # メインアプリケーション
│   ├── constants.py                     # 定数定義
│   ├── requirements.txt                 # 依存ライブラリ
│   ├── logic/
│   │   ├── api_client.py                # Gemini API通信（清書モード対応）
│   │   ├── yaml_generator.py            # YAML生成ロジック
│   │   ├── file_manager.py              # ファイル操作・タイトル合成
│   │   ├── character.py                 # キャラクター処理
│   │   └── usage_tracker.py             # API使用量トラッキング
│   └── ui/
│       ├── base_settings_window.py      # 設定ウィンドウ基底クラス
│       ├── character_sheet_window.py    # キャラデザイン設定
│       ├── pose_window.py               # ポーズ設定
│       ├── effect_window.py             # エフェクト設定
│       ├── background_window.py         # 背景設定
│       ├── decorative_text_window.py    # 装飾テキスト設定
│       ├── text_overlay_placement_window.py  # テキスト配置設定
│       ├── scene_builder_window.py      # シーンビルダー
│       ├── four_panel_window.py         # 4コマ漫画設定
│       └── manga_composer_window.py     # 漫画ページコンポーザー
├── template.yaml                        # テンプレートファイル
├── run_app.command                      # macOS/Linux起動スクリプト
├── run_app_windows.bat                  # Windows起動スクリプト
├── build_app.sh                         # PyInstallerビルドスクリプト
├── manga_generator.spec                 # PyInstaller設定ファイル
└── README.md
```

## YAMLテンプレート

各出力タイプは対応するYAMLテンプレートに準拠:

| テンプレート | 用途 |
|--------------|------|
| character_basic.yaml | キャラデザイン（三面図） |
| character_pose.yaml | ポーズ付きキャラ |
| character_effect.yaml | エフェクト追加 |
| background_template.yaml | 背景生成 |
| ui_text_overlay.yaml | 装飾テキスト |
| scene_composite.yaml | バトルシーン合成 |
| story_scene_composite.yaml | ストーリーシーン合成 |
| boss_raid_composition.yaml | ボスレイド合成 |

## 依存ライブラリ

- customtkinter - モダンなUIフレームワーク
- PyYAML - YAML処理
- Pillow - 画像処理・タイトル合成
- google-genai - Google Gemini API

## ブランチ構成

| ブランチ | 説明 |
|----------|------|
| main | 段階的生成パターン（推奨） |
| oldpattern | 旧1画面完結パターン（バックアップ） |

## 注意事項・免責事項

### ポーズ三面図の左右整合性について（技術的制限）

Step4「ポーズ三面図」で生成される正面・横・背面の三面図において、**左右の整合性は保証されません**。

例えば「右手を前に出すポーズ」を指定した場合：
- 正面図では右手が前に出ている
- しかし背面図では左手が前に出てしまう

このような不整合が発生する可能性があります。

**原因**: 現在の画像生成AIは、入力画像を「3Dモデル」として認識しておらず、「2Dピクセルの集合」として処理しています。そのため、「キャラクターの右手」と「画面右側に見える手」を正しく対応付けることができません。

**対処法**:
- 生成結果を確認し、問題がある場合は再生成してください
- 左右の整合性が重要な場合は、画像編集ソフトで手動修正することを推奨します
- 将来的にAI技術が進歩すれば改善される可能性があります

### 参考画像の著作権について

本アプリの以下の機能を使用する際は、著作権についてご注意ください：

- **Step3「参考画像から衣装を着せる」**: 参考画像から衣装のみをキャプチャ
- **Step4「参考画像のポーズをキャプチャ」**: 参考画像からポーズのみをキャプチャ
- **背景生成「参考画像から背景を生成」**: 参考画像から背景をキャプチャ・変形

**注意事項：**

- **著作権の確認**: 参考画像として使用する画像の著作権は、必ずユーザー自身で確認・管理してください
- **許諾の取得**: 第三者が著作権を持つ画像を使用する場合は、事前に適切な許諾を取得してください
- **商用利用**: 商用目的で使用する場合は、参考画像の権利関係を特に慎重に確認してください
- **自己責任**: 参考画像の使用に起因する著作権侵害その他の問題について、本アプリ開発者は一切の責任を負いません

### AI生成画像の利用について

- AI生成画像の著作権・利用規約は、Google Gemini APIの利用規約に従います
- 生成された画像の利用範囲・制限については、最新のGoogle AI利用規約をご確認ください
- 本アプリはビジネス用途（商品カタログ、マニュアル、教育資料など）での利用を想定していますが、生成内容の適切性はユーザーの判断と責任において確認してください

### 免責事項

- 本アプリは「現状有姿」で提供されており、動作保証はありません
- API仕様の変更により、一部機能が正常に動作しなくなる可能性があります
- 生成結果の品質・内容について、開発者は保証しません

## ライセンス

MIT License

## 関連プロジェクト

- [NanoBanana Pro](https://github.com/tokumeishatyo/NanoBanana) - シンプルな画像生成アプリ
- [Manga-Generator](https://github.com/tokumeishatyo/Manga-Generator) - 4コマ漫画プロンプト作成ツール
